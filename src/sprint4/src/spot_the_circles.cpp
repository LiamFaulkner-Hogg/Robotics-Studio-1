...
public:
    ...
    CylinderDetector() : Node("cylinder_detector"), map_received_(false), cylinder_diameter_(0.3), tf_buffer_(this->get_clock()), tf_listener_(std::make_shared<tf2_ros::TransformListener>(tf_buffer_))
    {
        ...
        civilian_publisher_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("/civilian", 10);
    }
...
private:
    ...
    void scanCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
    {
        ...
        if (detected_circles_.empty())
        {
            ...
            {
                RCLCPP_INFO(this->get_logger(), "INJURED PERSON DETECTED AT (%f, %f)", points.first, points.second);
                publishToOccupancyGrid(points.first, points.second);
                publishInjuredCivilianLocation(points.first, points.second);
            }
    ...
    void checkNewPoints(std::vector<std::pair<double, double>> circle_points)
    {
        ...
        for (auto &new_point : circle_points)
        {
            ...
            if (is_new)
            {
                ...
                publishInjuredCivilianLocation(new_point.first, new_point.second);
            }
        }
    }
    ...
    std::vector<std::pair<double, double>> detectCircleClusters(std::vector<std::pair<double, double>> points)
    {
        ...
        const int min_points_in_cluster = 10; // Minimum number of points to form a valid cluster (was 6 for turtlebot)
    ...
    void publishToOccupancyGrid(double centre_x, double centre_y)
    {
        ...
        int x_min = std::floor((centre_x - r) / map_msg_.info.resolution);
        int x_max = std::floor((centre_x + r) / map_msg_.info.resolution);
        int y_min = std::floor((centre_y - r) / map_msg_.info.resolution);
        int y_max = std::floor((centre_y + r) / map_msg_.info.resolution);

        // Prepare the data array for the update
        msg.data.reserve((x_max - x_min + 1) * (y_max - y_min + 1));
        msg.x = static_cast<int>(((centre_x - r) - map_msg_.info.origin.position.x) / map_msg_.info.resolution);
        msg.y = static_cast<int>(((centre_y - r) - map_msg_.info.origin.position.y) / map_msg_.info.resolution);
        ...
        for (int i = x_min; i <= x_max; ++i)
        {
            for (int j = y_min; j <= y_max; ++j)
            {
                ...
                double cell_x = (i + 0.5) * map_msg_.info.resolution; // Center of the cell
                double cell_y = (j + 0.5) * map_msg_.info.resolution; // Center of the cell
    ...
    void publishInjuredCivilianLocation(double centre_x, double centre_y)
    {
        geometry_msgs::msg::PoseStamped msg;

        msg.header.frame_id = "civilian";
        msg.header.stamp = this->get_clock()->now();

        msg.pose.position.x = centre_x;
        msg.pose.position.y = centre_y;
        msg.pose.orientation.z = 1;
        msg.pose.orientation.w = 1.0;

        civilian_publisher_->publish(msg);
    }
    /**
     * @brief Converts an image to an occupancy grid.
     * @param image cv::Mat to be converted to Occupancy Grid
     * @param grid Occupancy Grid output
     */
    void imageToOccupancyGrid(const cv::Mat &image, nav_msgs::msg::OccupancyGrid &grid)
    {
        for (int y = 0; y < image.rows; y++)
        {
            for (int x = 0; x < image.cols; x++)
            {
                ...
                else if (pixel == cv::Vec3b(255, 255, 255))
                {
                    grid.data[index] = 0; // Free
                }
                else if (pixel == cv::Vec3b(0, 255, 0))
                {
                    grid.data[index] = 100; // Occupied
                }
                else
    ...
    rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr civilian_publisher_;
